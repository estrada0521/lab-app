...
from .plot_runtime.presets import default as _plot_rc_defaults
...
# 先頭付近の mpl.rcParams.update … 全 ax 共通（フォント・目盛り・グリッド線種等）。個別の軸ラベルや lim は _AX_SPECS で編集。
...
def _embedded_global_rcparams_source() -> str:
    """plot.py 先頭にそのまま埋める rcParams ブロック（既定値は plot_runtime.presets.default と同期）。"""
    blob = pprint.pformat(dict(_plot_rc_defaults.RCPARAMS), width=88, sort_dicts=False)
    return (
        "# ---- 全 ax 共通: matplotlib rcParams（編集可。lab 側の既定は plot_runtime.presets.default）----\n"
        "_GLOBAL_MATPLOTLIB_RC: dict[str, object] = "
        + blob
        + "\n"
        "mpl.rcParams.update(_GLOBAL_MATPLOTLIB_RC)\n\n"
    )
...
def generate_plot_py(root: Path, rows: int, cols: int, cells: list) -> str:
    ...
    lines.append(_embedded_global_rcparams_source())
