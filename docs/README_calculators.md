...
`required_parameters_detail` は parameter 名の宣言だけにしてください。
fallback や alias のような解決経路は host 側の共通 resolver の責務であり、`calculator.json` には書きません。
...
calculator は metadata object と、host が前処理した `resolved_params` を受け取ります。

material / sample / rawdata の source of truth 自体は calculator の外にあります。
...
- calculator parameter の解決は host 側が `context.raw_meta["calc"]`, `context.sample_meta`, `context.material_meta` から行う
- calculator 本体は `context.resolved_params` を読む
- fallback logic は host 側の 1 箇所の helper に寄せる
- 必要なら user-facing な missing dependency message 用 helper を別に持つ
- output metadata には key 名だけでなく解決済みの値を残す

calculator parameter を `exp` metadata に隠さないでください。複製ではなく、rawdata / sample / material のどこを canonical source にするかを明確にしてください。
...
- `calculator.json` が実装と一致していること
  特に `required_parameters_detail` は parameter 名だけになっていること
